...
(function () {
  ...
  const css = `
    ...
    /* Attention pulse — radiating rings behind the button */
    .tgk-fb-btn::before,
    .tgk-fb-btn::after {
      content: '';
      ...
      opacity: 0.55;
      z-index: -1;
      animation: tgkFbPulse 2.4s cubic-bezier(0.4,0,0.2,1) infinite;
      pointer-events: none;
    }
    .tgk-fb-btn::after { animation-delay: 1.2s; }
    .tgk-fb-btn.tgk-fb-quiet::before,
    .tgk-fb-btn.tgk-fb-quiet::after { animation: none; opacity: 0; }
    @keyframes tgkFbPulse {
      0%   { transform: scale(1);   opacity: 0.55; }
      80%  { transform: scale(1.9); opacity: 0; }
      100% { transform: scale(1.9); opacity: 0; }
    }
    @media (prefers-reduced-motion: reduce) {
      .tgk-fb-btn::before, .tgk-fb-btn::after { animation: none; opacity: 0; }
    }
    ...
    .tgk-fb-tip {
      ...
      border-radius: 10px;
      box-shadow: 0 12px 32px rgba(19, 0, 50, 0.35);
    ...
    .tgk-fb-tip.active { opacity: 1; transform: translateX(0); }
    .tgk-fb-tip strong {
      display: block; font-size: 12.5px; font-weight: 600;
      color: #fff; margin-bottom: 3px; letter-spacing: -0.01em;
    }
    .tgk-fb-tip span { color: rgba(255,255,255,0.78); }
    .tgk-fb-tip::after {
    ...
    .tgk-fb-cat {
      ...
      background: #fff; color: rgba(19, 0, 50, 0.7);
      border-radius: 999px;
    ...
    .tgk-fb-cat:hover { border-color: #CBC2FF; color: #4C00FF; }
    .tgk-fb-cat.active {
      background: #4C00FF; color: #fff; border-color: #4C00FF;
    }
    .tgk-fb-badge {
  ...
  tip.className = 'tgk-fb-tip';
  tip.id = 'tgk-fb-tip';
  tip.setAttribute('role', 'tooltip');
  tip.innerHTML = `
    <strong>Share feedback on this screen</strong>
    <span>Click to pick a category (design, content, bug, other), drop a quick note, and we'll route it to the product team for review.</span>
  `;
  ...
  panel.innerHTML = `
    <div class="tgk-fb-header">
      ...
      <p>Tell us what's unclear, broken, or worth changing on this screen. Pick a category, leave a quick note, and the product team will see it in the review queue.</p>
    ...
    <div class="tgk-fb-form">
      ...
      <div class="tgk-fb-body">
        ...
        <div class="tgk-fb-cats" role="radiogroup" aria-label="Feedback category">
          <button type="button" class="tgk-fb-cat" role="radio" aria-checked="false" data-cat="design">Design</button>
          <button type="button" class="tgk-fb-cat" role="radio" aria-checked="false" data-cat="content">Content</button>
          <button type="button" class="tgk-fb-cat" role="radio" aria-checked="false" data-cat="bug">Bug</button>
          <button type="button" class="tgk-fb-cat" role="radio" aria-checked="false" data-cat="other">Other</button>
        </div>
  ...
  const elCats    = panel.querySelectorAll('.tgk-fb-cat');
  ...
  function openPanel() {
    ...
    btn.classList.add('tgk-fb-quiet'); // stop pulsing while open
    hideTip();
    refreshExportButton();
  ...
  function closePanel() {
    ...
    btn.classList.remove('tgk-fb-quiet'); // resume attention pulse
    ...
    setTimeout(() => {
      ...
      elCats.forEach(c => { c.classList.remove('active'); c.setAttribute('aria-checked', 'false'); });
    }, 220);
  }
  function showTip() {
    if (panel.classList.contains('active')) return;
    tip.classList.add('active');
    btn.setAttribute('aria-describedby', 'tgk-fb-tip');
  }
  function hideTip() {
  ...
  function refreshSubmitState() {
    elSubmit.disabled = !(selectedCategory && elMsg.value.trim().length >= 3);
  }
  ...
  elMsg.addEventListener('input', refreshSubmitState);
  elCats.forEach(cat => {
  ...
  // Show tooltip once shortly after load to draw attention
  setTimeout(() => {
