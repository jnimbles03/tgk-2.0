...
- **Flipbooks** (`/flipbooks/`) — DocuSign CLM Procurement Demo (51 frames), Vendor Agreement Management Renewals (14), Agreement Manager (18), AI-Assisted Review / Iris (19), Workspaces (19), Web Forms / Tally Bank (23), Workflow Builder Workflow Templates (6), IAM for Sales + Agentforce (53)
- **Marketing decks** — *Procurement Intake and Orchestration* (front of lifecycle), *Vendor Agreement Management* (back of lifecycle)
- **Demo videos** — CLM Procurement Demo (contract management specialist drafts → workflow → vendor → legal redline → executive → sign → store), Mary/Ventura → Stellar Logical Agentforce arc (NDA gap → MSA from CRM → AI playbook redline → customer AI summary → Agreement Manager renewal foresight)

Both stories follow the canonical **5-scene spine**: Sender · Identity · Signing · Agreement Manager · Workspace. The procurement story leans on Workflow Builder orchestration end-to-end; the sales story leans on Agentforce + IAM for Sales as the Salesforce-native entry point. AI-Assisted Review by Iris appears in **both** stories as the bridge between drafting and signing.
...
|---|---|---|
| Scene 1 — Sender | Workflow Builder + Agreement Manager | Renewal alert dashboard surfaces a contract 60 days before expiration. Workflow Builder fans out a renewal package: pre-filled redline, supplier brief, current obligation summary. |
...
| Scene 4 — Agreement Manager | Agreement Manager + Iris | Executed renewal lands in Agreement Manager. Obligation extraction picks up the new term, the new payment schedule, the new SLA credits. Iris answers "what's the new auto-renewal notice window?" with a citation back to clause 14.2. |
...
### Scene 1 — Sender (Agreement Desk · Workflow Builder)
...
2. **Workflow Builder routes the renewal.** Click the row. Workflow Builder fan-out diagram animates: pulls current contract from Agreement Manager, identifies signers (supplier + Avis Legal + Avis VP Operations), pre-stages the redline using the *previous* contract as the baseline.
3. **Pre-fill from Coupa + Agreement Manager.** Supplier address, tax ID, current rate card auto-pull from Coupa. Last-12-months SLA performance auto-pulls from Agreement Manager (since AVMS captures obligations + actuals against them).
...
5. **Handoff to supplier.** Workflow Builder emits a signed Workflow Builder link to the supplier's authorized signer. The activity log at the bottom of the deck's slide 5 mockup shows the email sent.
...
### Scene 4 — Agreement Manager (post-signature insight)
1. **Renewal hits Agreement Manager.** AI-assisted banner: "Agreement extracted by Iris — 47 fields captured." The new MSA shows up in the agreement list with the new term, new rate card, new SLA credits.
...
3. **Renewal date set.** Auto-set 60 days before next expiration. Agreement Manager schedules the next renewal alert.
...
5. **Reporting.** Agreement Manager dashboard updates: 1 of 12 renewals closed, 11 in-flight. Aggregate value of closed renewals: $1.4M. Average days from alert to executed: 19 (down from 73 pre-IAM, per the deck's productivity claim).
...
4. **Conversational drill-down.** Avis Legal asks "show me all force-majeure invocations in the last 24 months across this supplier" — Iris answers from Agreement Manager, scoped by the workspace's contract family.
...
- *New Supplier Onboarding* (the other procurement use case in step 2) is a cousin — same 5 scenes, but Scene 1 is Web Forms intake instead of Workflow Builder renewal alert, and Scene 4 is "new supplier in Agreement Manager" instead of "renewal diff." Worth building next, since the deck explicitly shows the Web Forms / Workflow Builder / AI-Assisted Review / eSignature stack on slide 6.
...
| Spine slot | Product | What you see |
|---|---|---|
| Scene 1 — Sender | Agentforce + IAM for Sales | Mary asks Agentforce "does Stellar Logical have an active NDA?" inside Salesforce. Agentforce queries Agreement Manager, finds an expired NDA, offers to send a new one pre-filled from CRM. One click → sent. |
...
| Scene 4 — Agreement Manager | Agreement Manager + Salesforce-embedded repository | DocuSign Agreements tab in Salesforce shows every executed NDA, MSA, order. Searchable by clause. Feeds forecasting, renewal planning, margin analysis. |
| Scene 5 — Workspace | Agentforce + Agreement Manager (renewal foresight) | 11 months later, Mary preps for QBR. "Agentforce, top renewals next 6 months by contract value?" — surfaces them with notice deadlines pulled from Agreement Manager. She initiates renewal discussions ahead of schedule. |
...
2. **Asks Agentforce.** Conversational query inside the opportunity panel: "does Stellar Logical have an active NDA?" Agentforce queries Agreement Manager behind the scenes.
...
### Scene 4 — Agreement Manager (post-signature, in Salesforce)
...
3. **Feeds forecasting.** Mary's pipeline view now shows committed renewal dates (from Agreement Manager), not just expected close dates. Sales ops uses this for capacity planning.
4. **Margin analysis.** Finance pulls aggregate concession data ("how often do we agree to net-60 on >$1M deals?") from Agreement Manager, not from spreadsheet exports.
...
- *CPQ-to-Sign* and *Channel Partner* (the other two sales use cases in step 2) are deferred until the IAM for Sales + Agentforce flipbook narrative is fully built, since they share Scene 4 (Agreement Manager-in-CRM) but diverge sharply at Scenes 1-3.
...
|---|---|---|
| **Workflow Builder** | Renewal package fan-out (Scene 1) | — |
...
| **Agreement Manager** | Scene 4 (obligation diff, Iris citations) | Scene 4 (Salesforce-embedded repository) |
...
Both stories share Scenes 2-4 architecturally. The differentiation is at Scene 1 (Workflow Builder orchestration vs. Agentforce conversational) and Scene 5 (operational workspace vs. renewal foresight in CRM).
...
4. **Importer support for new VERTICALS keys** — `scripts/import_storylines.py` already handles arbitrary keys, but the unified picker's CATALOG (in `docs/experiments/index-unified.html`) would need entries if we want the new stories surfaced through both the function-first (geos) and vertical-first (unified) front doors.
